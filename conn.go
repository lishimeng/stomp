...
func processLoop(c *Conn, writer *frame.Writer) {
	...
	var readTimeoutEnable = false
	var writeTimeoutEnable = false
	var readTimeoutDuration time.Duration
	var writeTimeoutDuration time.Duration

	if c.readTimeout > 0 {
		readTimeoutDuration = time.Duration(float64(c.readTimeout) * c.hbGracePeriodMultiplier)
		readTimeoutEnable = true
	} else {
		c.readTimeout = time.Hour
	}
	if c.writeTimeout > 0 {
		writeTimeoutDuration = c.writeTimeout
		writeTimeoutEnable = true
	} else {
	...
	for {
		...
		if writeTimer == nil {
			writeTimer = time.NewTimer(writeTimeoutDuration)
		}
